...
<body>
    <div class="container">
        ...
        <div class="content">
            <h2>Wizyta została przeniesiona</h2>
            <p>Cześć {{ $oldVisit->user->name ?? 'Kliencie' }},</p>

            <p class="text-own">Twoja wizyta została przeniesiona na nowy termin </p>
            {{-- <p>Stary termin: <span class="text-list-data">{{ \Carbon\Carbon::parse($oldVisit->date)->format('d.m.Y') }} {{ \Carbon\Carbon::parse($oldVisit->start_time)->format('H:i') }}</span></p> --}}
            <p>Nowy termin: <span class="text-list-data">{{ \Carbon\Carbon::parse($newVisit->date)->format('d.m.Y') }} {{ \Carbon\Carbon::parse($newVisit->start_time)->format('H:i') }}</span></p>
            ...
            <p class="before-footer">Pozdrawiamy!<br>Zespół Fizjoterapia Kaczmarek</p>
        </div>
