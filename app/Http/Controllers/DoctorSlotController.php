...
public function getSlotsRange(Request $request)
{
    ...
    // 🔹 Pobierz grafiki pracy dla danego zakresu i lekarza
    $schedules = \App\Models\Schedule::query()
        ->whereBetween('date', [$from, $to])
        ->when($doctorId, fn($q) => $q->where('doctor_id', $doctorId))
        ->get()
        ->groupBy(fn($s) => $s->doctor_id . '|' . $s->date);

    // 🔹 Filtrowanie slotów, które są w zakresie godzin pracy
    $slots = $slots->filter(function ($slot) use ($schedules) {
        $key = $slot->doctor_id . '|' . $slot->date;
        $doctorSchedules = $schedules[$key] ?? collect();

        // Jeśli lekarz nie ma grafiku danego dnia — odrzuć slot
        if ($doctorSchedules->isEmpty()) {
            return false;
        }

        // Sprawdź, czy jakikolwiek grafik pokrywa się z godziną slotu
        ...
        return false; // Slot poza grafikiem -> odrzuć
    })->values();
